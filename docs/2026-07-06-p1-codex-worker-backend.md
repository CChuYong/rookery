...
**Sandbox network note (final review M3, RESOLVED by P1.5 Track E):** spawn-time `workspace-write` (string form) leaves network access to the user's codex config default, while a live `setPermissionMode` re-applies it as the object form pinned `networkAccess: true` — same rookery mode, different network exposure by path. Resolved: always network-on, expressed as an explicit per-turn policy so spawn and live re-apply agree (see docs/2026-07-06-p15-codex-followups.md).
