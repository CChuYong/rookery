...
| `capabilities.snapshot` | ✓ | `target: {kind:"rookery",provider,agent}\|{kind:"repo",id,provider,agent}\|{kind:"session"\|"worker",id}` | read a preview or live target's effective capability inventory | `capabilities.snapshot.result` |
...
- **`capabilities.snapshot` target authority:** the target is a strict discriminated union. Live targets are exactly `{kind:"session"|"worker",id}`; the daemon resolves provider, label, cwd, repository, origin, home session, and worker worktree from persisted rows. Preview targets are exactly `{kind:"rookery",provider,agent}` or `{kind:"repo",id,provider,agent}`, where provider is `claude|codex` and agent is `master|worker`; injected cwd/origin or other runtime fields are rejected. A repository preview resolves its label and cwd from the exact registered repository id, while a Rookery preview has no cwd and intentionally skips provider-native inventory. Unknown targets are correlated errors, never empty successful snapshots. Preview replies expose deterministic desired state but omit runtime/applied state and invocation metadata and never create runtime or Codex-home files. Live replies merge Rookery built-ins, provider inventory, and managed entries; `desiredRevision` is deterministic, `desiredBlocked` reports fail-closed required state, and `appliedRevision` plus launchable-entry status reflects actual runtime drift. Independent provider probe failures remain in `diagnostics[]` while successful entries stay visible.
