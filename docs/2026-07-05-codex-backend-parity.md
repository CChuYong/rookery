...
> Status 2026-07-05+: **P0 (seam extraction) implemented** — `src/core/agent-backend.ts` (port) + `src/core/claude-backend.ts` (adapter); Worker/MasterAgent/SessionManager are SDK-import-free (guarded by `test/core/provider-neutral.test.ts`). P1 (Codex worker backend) implemented 2026-07-06 — see docs/2026-07-06-p1-codex-worker-backend.md. P1.5 implemented 2026-07-06 — desktop provider UX, per-turn pricing with verified RATES, in-app `codexApiKey` via `CODEX_HOME`, fork timeout, always-network-on workspace-write; see docs/2026-07-06-p15-codex-followups.md. **P2 (Codex master) implemented 2026-07-06** — master-on-codex: daemon MCP bridge + per-turn ephemeral child + AskUserQuestion def + `sessions.provider` routing; see docs/2026-07-06-p2-codex-master.md. **P2.5 implemented 2026-07-06** — argv-token hardening (bridge URL moved out of child argv into a per-session `CODEX_HOME` `config.toml`), a per-turn inactivity watchdog, slack-origin codex sessions (`slackProvider` setting), desktop new-session provider UI + `codexMasterModel` field; see docs/2026-07-06-p25-codex-hardening.md. **P3 (#1 codex master fork + #8 automation-origin codex) implemented 2026-07-06** — see docs/2026-07-06-p3-codex-fork-automation.md. **P3-remaining hardening done 2026-07-06** — see docs/2026-07-06-p3r-codex-hardening-finish.md: the Codex track (P0→P3+hardening) is complete — workers, masters, slack, automation, fork all live; handshake timeout, orphan GC, delete-hook, desktop settings fields landed. Remaining backlog = **non-Codex only**: Claude `total_cost_usd` cumulative-vs-per-turn audit, `sessions.test.tsx` teardown flake, + the older deferred threads (dockable panes, UI/UX audit follow-ups, cross-platform Phase B). **Cost budget guard (2026-07-06): partly closed** — a lifetime USD cost budget (`costBudgetUsd`, default off) now stops a worker / warns a master at `cumCostUsd >= budget`, settable per-spawn/per-automation or via the `workerCostBudgetUsd` settings default; see docs/2026-07-06-cost-budget-guard.md. A lifetime **turn** budget guard is still future. **Model+effort picker (2026-07-06): landed** — the desktop Codex model/effort fields are now dropdowns driven by the app-server `model/list` catalog (Row 7 above) via the daemon `codex.models.list` provider (`src/core/codex-models-provider.ts`, cached, free-text fallback), per-model reasoning-effort selector, across all 4 surfaces; replaces the free-text codex model inputs. See docs/superpowers/specs/2026-07-06-codex-model-picker-design.md.
