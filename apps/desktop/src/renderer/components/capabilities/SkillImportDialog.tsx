  ...
  return (
    <div className={cn("fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-5 backdrop-blur-sm", closing ? "motion-safe:animate-[overlay-out_130ms_ease-in]" : "motion-safe:animate-[overlay-in_160ms_ease-out]")}>
      <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="skill-import-title" className={cn("w-[min(620px,96vw)] overflow-hidden rounded-xl border border-line bg-surface shadow-2xl", closing ? "motion-safe:animate-[dialog-out_140ms_ease-in]" : "motion-safe:animate-[dialog-in_180ms_ease-out]")}>
        ...
        <form className="space-y-4 px-5 py-4" onSubmit={(event) => { event.preventDefault(); submit(); }}>
          <label className="block text-[10.5px] font-medium text-fg-dim">{t("capabilityCatalog.name")}<Input autoFocus className="mt-1" size="sm" value={displayName} onChange={(event) => { const value = event.target.value; setDisplayName(value); if (!idTouched) setId(slugMcpId(value)); }} /></label>
          <label className="block text-[10.5px] font-medium text-fg-dim">{t("capabilityCatalog.id")}<Input className="mt-1" size="sm" value={id} onChange={(event) => { setIdTouched(true); setId(event.target.value); }} /></label>
          <label className="block text-[10.5px] font-medium text-fg-dim">{t("capabilityCatalog.description")}<Textarea className="mt-1" rows={3} size="sm" value={description} onChange={(event) => setDescription(event.target.value)} /></label>
          <div>
            <span className="text-[10.5px] font-medium text-fg-dim">{t("capabilityCatalog.skillDirectory")}</span>
            <div className="mt-1 flex items-center gap-2"><Input size="sm" readOnly aria-label={t("capabilityCatalog.skillDirectory")} value={sourcePath} /><Button type="button" variant="outline" size="sm" disabled={busy} onClick={() => { void props.pickDirectory().then((selected) => { if (selected) setSourcePath(selected); }); }}><FolderOpen size={13} /> {t("capabilityCatalog.chooseDirectory")}</Button></div>
          </div>
          {error && <p role="alert" className="rounded-md border border-fail/30 bg-fail/5 p-3 text-[11px] text-fail">{error}</p>}
        </form>
        <div className="flex items-center gap-3 border-t border-line px-5 py-3"><p className="min-w-0 flex-1 text-[10.5px] text-run">{t("capabilityCatalog.skillTrustNotice")}</p><Button variant="outline" disabled={busy} onClick={dismiss}>{t("common.cancel")}</Button><Button variant="primary" loading={busy} onClick={submit}>{t("capabilityCatalog.importSkill")}</Button></div>
      </div>
    </div>
  );
}
