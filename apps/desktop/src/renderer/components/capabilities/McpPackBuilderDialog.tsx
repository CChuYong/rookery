...
import type { CapabilityMcpPackCreateInput, CapabilityMcpPackCreateResult } from "@daemon/core/capabilities/types.js";
...
export interface McpPackBuilderDialogProps {
...
function initialDraft(): McpPackDraft {
  return {
    displayName: "",
    id: "",
    version: "1.0.0",
...
export function McpPackBuilderDialog(props: McpPackBuilderDialogProps): JSX.Element {
  ...
  const submit = (): void => {
    ...
    void props.create(compiled.input).then((result) => {
    ...
    <div className={cn("fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-5 backdrop-blur-sm", closing ? "motion-safe:animate-[overlay-out_130ms_ease-in]" : "motion-safe:animate-[overlay-in_160ms_ease-out]")}> 
      <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="mcp-pack-builder-title" className={cn("flex max-h-[92vh] w-[min(960px,96vw)] flex-col overflow-hidden rounded-xl border border-line bg-surface shadow-2xl", closing ? "motion-safe:animate-[dialog-out_140ms_ease-in]" : "motion-safe:animate-[dialog-in_180ms_ease-out]")}> 
        <div className="flex items-start gap-3 border-b border-line px-5 py-4">
          <div className="min-w-0 flex-1"><h2 id="mcp-pack-builder-title" className="text-[15px] font-semibold text-fg">{t("mcpPackBuilder.title")}</h2><p className="mt-1 text-[11px] text-muted">{t("mcpPackBuilder.description")}</p></div>
        ...
        <form className="min-h-0 flex-1 overflow-y-auto px-5 py-4" onSubmit={(event) => { event.preventDefault(); submit(); }}>
          <section className="grid gap-3 rounded-lg border border-line bg-base/25 p-3 md:grid-cols-2">
            <Field label={t("mcpPackBuilder.packName")}><Input autoFocus size="sm" aria-label={t("mcpPackBuilder.packName")} value={draft.displayName} onChange={(event) => { const displayName = event.target.value; setDraft((current) => ({ ...current, displayName, id: idTouched ? current.id : slugMcpId(displayName) })); }} /></Field>
            ...
            <Field label={t("mcpPackBuilder.repo")}><Select size="sm" aria-label={t("mcpPackBuilder.repo")} value={draft.repoId} onChange={(event) => setDraft((current) => ({ ...current, repoId: event.target.value }))}><option value="">{t("mcpPackBuilder.repoPlaceholder")}</option>{props.repos.map((repo) => <option key={repo.id} value={repo.id}>{repo.label}</option>)}</Select></Field>
            <Field label={t("mcpPackBuilder.version")}><Input size="sm" aria-label={t("mcpPackBuilder.version")} value={draft.version} onChange={(event) => setDraft((current) => ({ ...current, version: event.target.value }))} /></Field>
            ...
            <fieldset className="md:col-span-2"><legend className="mb-1 text-[10.5px] font-medium text-fg-dim">{t("mcpPackBuilder.agents")}</legend><div className="flex gap-4">{(["master", "worker"] as const).map((agent) => <label key={agent} className="flex items-center gap-2 text-[11.5px] text-fg"><input type="checkbox" checked={draft.agents.includes(agent)} onChange={(event) => setDraft((current) => ({ ...current, agents: event.target.checked ? [...current.agents, agent] : current.agents.filter((item) => item !== agent) }))} />{t(`mcpPackBuilder.${agent}`)}</label>)}</div></fieldset>
          </section>
          ...
          <Button className="mt-3" variant="outline" size="sm" onClick={() => setDraft((current) => ({ ...current, servers: [...current.servers, createEmptyMcpServerDraft("streamable-http")] }))}><Plus size={12} /> {t("mcpPackBuilder.addServer")}</Button>
