...
- **Masters can also run on Codex** (`session.create provider:"codex"`; protocol-only for now — no desktop new-session UI): each turn spawns a **per-turn ephemeral `codex app-server` child** and calls `thread/start`/`thread/resume` with **fresh `developerInstructions` every turn** (= the per-turn `buildSystemPrompt()` injection Codex otherwise lacks). Tools reach the child over the daemon's **MCP bridge** (`/mcp/<session-token>` mounted on the daemon http port; `src/daemon/mcp-bridge.ts`), not in-process MCP servers. ⚠️ **bypassPermissions only** — restricted sandboxes silently swallow the bridge's turn-scoped MCP calls (`item/started` fires, `tools/call` never arrives, the turn hangs); non-bypass permission modes are rejected at turn start. The bridge URL token is also visible in the child's argv (`-c mcp_servers.rookery.url=...`, readable via `/proc/*/cmdline` by other local users on multi-user Linux) — a trusted-machine assumption, same caveat class as `bypassPermissions` itself; P2.5 moves it into a per-session `CODEX_HOME` `config.toml`. See `docs/2026-07-06-p2-codex-master.md`.
