...
- **Codex worker auth** rides on the user's `~/.codex/auth.json` (`codex login`) — the app-server child does NOT read `CODEX_API_KEY` (verified against rust-v0.142.5); an in-app key setting is P1.5 (CODEX_HOME redirection + account/login/start).
