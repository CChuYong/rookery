...
**Choosing a backend (Claude vs Codex):** both backends reach the same in-process tools (memory, repos, fleet, schedule) and run the same master/worker orchestration — what differs is the model and the permission model. Claude supports all permission modes; **Codex masters are bypassPermissions-only** (they auto-approve tool use, so run them only in trusted paths). Each backend's model, effort, and API-key defaults live in **Settings → Models** (a Claude/Codex sub-toggle). Pick the backend per session/worker/automation/Slack thread — or, from the CLI, with `--provider claude|codex` (see `rookery --help`).
